...
- For a stable, readable chart, display anchors are derived from the latest 180-candle window separately from the internal structure lines used for signals. Bullish display support starts at the departure candle immediately after the lowest wick and connects to the shallowest confirmed higher low. Bearish display resistance starts at the highest wick and connects to the shallowest confirmed lower high.
- Once CHoCH breaks the opposite display line, that broken line is frozen at the first confirmed pivot after the break rather than drifting to later candles. In the reviewed sample this derives bullish 05:29 to 07:57 and bearish 07:05 to 07:42 without timestamp-specific rules.

...
- For a stable, readable chart, display anchors are derived separately from the internal structure lines used for signals. One-minute display structure uses the loaded 300-candle chart history; 15-minute context uses a rolling 240-candle structural window so an older completed cycle cannot control the current line indefinitely.
- Bullish display support begins at the lowest wick and connects to the shallowest confirmed higher low. Bearish display resistance begins at the highest wick and connects to the shallowest confirmed lower high. The active structural cycle caps the one-minute endpoints at its latest minor trend-line break, then adjusts again when a new confirmed cycle becomes available.
- The reviewed one-minute sample derives bullish 05:28 to 07:57 and bearish 07:05 to 08:07. The reviewed 15-minute sample derives bullish September 2 10:45 to September 3 05:15 and bearish September 1 05:45 to September 3 07:00. Every anchor uses its wick value; none uses timestamp-specific rules.
...
The 08:07 close through the adjusted one-minute bearish resistance is retained as a minor bullish CHoCH chart marker. Minor display CHoCH markers document a lower-level line break but do not independently create a second trade or alter the primary strategy bias. The exact wick rule also identifies the reviewed 07:43 bearish FVG at 77,992.0-78,007.3 and 08:28 bullish FVG at 77,863.4-77,909.4.

The automated indicators do not prevent chart navigation. Use the mouse wheel to zoom, drag the chart to move through candle history, drag the time or price axis to rescale, and double-click an axis to reset it. Navigating away pauses automatic real-time following; **Latest candles** returns to the live edge. Overlay replacement is deferred while a pointer drag is active so live analysis cannot invalidate the chart gesture.
